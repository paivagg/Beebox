...
const Products: React.FC = () => {
  ...
  return (
    <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden">
      ...
      <main className="flex-grow px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-24">
            ...
            <div className="flex items-stretch justify-between gap-4">
              <div className="flex flex-[2_2_0px] flex-col gap-2 justify-between">
                <div className="flex flex-col gap-1">
                  <p className={`text-sm font-medium leading-normal ${product.stock <= 5 ? 'text-red-400' : 'text-text-secondary-dark'}`}>
                    Estoque: {product.stock} un
                  </p>
                  <p className="text-base font-bold leading-tight text-white line-clamp-2">{product.name}</p>
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-normal leading-normal text-gray-400">{product.category}</p>
                    <span className="material-symbols-outlined text-gray-500 text-sm">edit</span>
                  </div>
                </div>
                <div className="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-lg bg-white/5 border border-white/5 px-4 text-sm font-medium leading-normal text-white h-9 w-fit mt-2">
